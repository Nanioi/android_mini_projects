package com.nanioi.electronicframe
    ...
    private fun startTimer() {
        timer = timer(period = 5 * 1000) {
            runOnUiThread {
                ...
                val next =
                    if (photoList.size <= currentPosition + 1) 0 else currentPosition + 1  //마지막 이미지일 경우 처음으로

                backgroundPhotoImageView.setImageURI(photoList[current])

                photoImageView.alpha = 0f // 투명도 -> 투명하게 이미지가 안보임
                photoImageView.setImageURI(photoList[next])
                photoImageView.animate()//이미지 애니메이션효과
                    .alpha(1.0f)
                    .setDuration(1000)
                    .start()
